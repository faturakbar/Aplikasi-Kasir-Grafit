...
JOIN kategori_barang ON barang.kode_kategori = kategori_barang.kode_kategori where
(no_nota BETWEEN '$from' AND '$to')  order by no_nota ");
...
$data = mysqli_query($con," SELECT SUM(sub_total) as total_jual from detail_penjualan JOIN barang ON barang.id_barang = detail_penjualan.id_barang
JOIN kategori_barang ON barang.kode_kategori = kategori_barang.kode_kategori where
(no_nota BETWEEN '$from' AND '$to')  order by no_nota ");
...
$pdf->Cell(5,0.7,"Pendapatan : "."Rp.".number_format($hasil['total_jual']).",-",0,0,'C');
}
